...
#  Some constants
DUCKLING_EXTRACTOR = "DucklingEntityExtractor"
...
def send_option(dispatcher: CollectingDispatcher, tracker: Tracker):
    ...
    if recipe is None:
        ...
        if input_channel == "socketio":
            img_message = {
                "type": "image",
                "payload": {"title": recipe["dish"], "src": recipe["pic"], "url": recipe["url"]},
            }
        else:
...
def send_link(dispatcher: CollectingDispatcher, tracker: Tracker):
    ...
    if input_channel == "socketio":
        ...
        dispatcher.utter_message(text=f'Here is the link to the full recipe {recipe["url"]}')
...
def check_matches(dispatcher: CollectingDispatcher, tracker: Tracker, value, slot):
    search_params = get_params(dispatcher, tracker)
...
class ResetMainDurationSlots(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        return [SlotSet("main", None), SlotSet("duration", None), SlotSet("satisfied", None)]
...
class RecipeForm(FormAction):
    ...
    def slot_mappings(self):
        ...
        mappings = {
            ...
            "duration": [
                ...
                self.from_entity(entity="pseudo_duration"),
            ],
            "dietary": [
                self.from_entity(entity="dietary"),
                self.from_intent(intent="deny", value=False),
    ...
    def validate_main(
        ...
        if "dietary" in ent or "duration" in ent or "pseudo_duration" in ent and "main" not in ent:
            value = False
        elif "main" in ent:
    ...
    def validate_duration(
        ...
        if isinstance(value, dict):
            ...
            if DUCKLING_EXTRACTOR in ent:
                # Then pull directly
                norm_duration = ent[DUCKLING_EXTRACTOR]["additional_info"]["normalized"]
            else:
                # Ping the duckling server
                duration = ent.values()[0]["value"]
            ...
            norm_duration = check_matches(dispatcher, tracker, norm_duration, "time2cook")
