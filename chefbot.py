...
def main():
    ...
    if args.mode == "full":
        ...
        time.sleep(2)
        up_website_args = []
        web_logger = create_log("update_web")
        subprocess.run(
            ["python3", "update_website.py"] + up_website_args, stdout=web_logger, stderr=web_logger
        )
    ...
    if args.mode == "local":
        subprocess.run(["open", "web-demo/index.html"]) 
...
def start_ngrok():
    start_p("ngrok", ["ngrok", "http", "-bind-tls=true", "5005"], "shell")
...
def start_action_server():
    start_p(
        "rasa_actions", ["rasa", "run", "actions", "--actions", "bot.src.actions", "--port", "5055"]
    )
